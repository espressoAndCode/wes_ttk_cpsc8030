#include <ttkIsocontour.h>

using namespace std;
using namespace ttk;

vtkStandardNewMacro(ttkIsocontour)

    int ttkIsocontour::doIt(vector<vtkDataSet *> &inputs, vector<vtkDataSet *> &outputs)
{
    ...
    isocontour_.setupTriangulation(triangulation);
    isocontour_.setWrapper(this);
    ...
    isocontour_.setInputDataPointer(inputScalarField->GetVoidPointer(0));

    vector<vector<float>> final_points;

    switch (inputScalarField->GetDataType())
    {
        ttkTemplateMacro(isocontour_.computeIsolines<VTK_TT>(SomeIntegerArgument, final_points));
    }
